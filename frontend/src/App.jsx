...
// 🚀 Code-Splitting para PCs de bajos recursos y móviles (carga bajo demanda)
const PublicTiendaVirtual = lazy(() => import('./PublicTiendaVirtual'))
const SuperAdminPortal = lazy(() => import('./SuperAdminPortal'))

function ViewLoadingFallback({ message = 'Cargando módulo...' }) {
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#0f172a', color: '#ffffff', gap: '14px', padding: '20px', textAlign: 'center' }}>
      <div style={{ fontSize: '36px' }}>🥩</div>
      <div style={{ fontSize: '15px', fontWeight: '700', color: '#cbd5e1' }}>{message}</div>
    </div>
  )
}
import { 
...
function PosAdminSystem() {
  ...
  if (currentUser.rol === 'superadmin') {
    return (
      <Suspense fallback={<ViewLoadingFallback message="Accediendo al Portal SuperAdmin..." />}>
        <SuperAdminPortal 
  ...
  return (
    ...
    onChange={(e) => setCalcPrecioPie(parseFormattedNumber(e.target.value))}
      ...
      {/* 📱 BARRA DE NAVEGACIÓN INFERIOR MÓVIL (BOTTOM NAVIGATION BAR ERGONÓMICA) */}
      <nav className="mobile-bottom-nav" aria-label="Navegación principal móvil">
        ...
        <button
          type="button"
          className={`mobile-bottom-nav-item ${activeTab === 'pedidos' ? 'active' : ''}`}
          onClick={() => { setActiveTab('pedidos'); setMobileMenuOpen(false); }}
        >
          <div style={{ position: 'relative', display: 'inline-flex' }}>
            <ClipboardIcon style={{ width: 22, height: 22 }} />
            {pedidosPendientesCount > 0 && (
              <span className="mobile-bottom-nav-badge">{pedidosPendientesCount}</span>
            )}
          </div>
          <span>Pedidos</span>
        </button>
        ...
        >
          <div style={{ position: 'relative', display: 'inline-flex' }}>
            <BoxIcon style={{ width: 22, height: 22 }} />
            {stockBajoAlerts.length > 0 && (
              <span className="mobile-bottom-nav-badge warning">{stockBajoAlerts.length}</span>
            )}
          </div>
          <span>Stock</span>
        </button>
        ...
        <button
          type="button"
          className={`mobile-bottom-nav-item ${activeTab === 'contabilidad' ? 'active' : ''}`}
          onClick={() => { setActiveTab('contabilidad'); setMobileMenuOpen(false); }}
        >
          <div style={{ position: 'relative', display: 'inline-flex' }}>
            <DollarIcon style={{ width: 22, height: 22 }} />
            <span 
              className={`mobile-caja-dot ${isCajaAbierta ? 'open' : 'closed'}`} 
              title={isCajaAbierta ? "Caja abierta" : "Caja cerrada"} 
            />
          </div>
          <span>Caja</span>
        </button>
        ...
        <button
          type="button"
          className={`mobile-bottom-nav-item ${mobileMenuOpen ? 'active' : ''}`}
...
// 🌐 Componente Raíz: Enrutamiento ligero que aísla completamente la Tienda Virtual del POS
function App() {
  ...
  const isPublicStore = 
    ...
    currentPath.includes('view=tienda')

  if (isPublicStore) {
    return (
      <Suspense fallback={<ViewLoadingFallback message="Cargando Tienda Virtual..." />}>
        <PublicTiendaVirtual />
      </Suspense>
    )
  }
